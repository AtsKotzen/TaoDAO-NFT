...
$(document).ready(async function(){
  ...
  TaoDAONFTTokenInstance = new web3.eth.Contract(abi.TaoDAONFTToken, TaoDAONFTTokenAddress);
  TaoDAONFTMarketplaceInstance = new web3.eth.Contract(abi.TaoDAONFTMarketplace, TaoDAONFTMarketplaceAddress);
...
$('#forSale').click(async ()=>{
  ...
  let activeCount = ifOfferDetailsDuplicatesRemoved.filter(item => !item.isSold && item.active).length;
  if(activeCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork for sale on TaoDAONFT, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
  recentlyPutForSale();
});
...
$('#notForSale').click(async ()=>{
  ...
  let inactiveCount = inactiveArtwork.filter(item => !item.active).length;
  if(inactiveCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork not for sale on TaoDAONFT, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
...
async function viewAll(){
  ...
  if(activeCount < 1 && inactiveCount < 1){
    $('.minted-wrapper').html(`<div class="no-art-for-sale shadow-sm">There is currently no artwork on TaoDAONFT, but you can change that <a class="gradient-text" href="create.html"> here!<a> 😎<div>`);
  }
  recentlyPutForSale();
  recentlyMintedAndNotOnSale();
};
...
function soldCardShareOptions(tokenAddress, id){
  ...
  let tweet = `https://twitter.com/intent/tweet?text=Check%20out%20this%20NFT%20on%20TaoDAONFT!&hashtags=TaoDAONFT%2Cbsc%2Cnonfungible%2Cdigitalasset%2Cnft&via=TaoDAONFT&url=${tokenPage}`;
  let post = `https://www.facebook.com/sharer/sharer.php?u=${tokenPage}%3F&quote=Check%20out%20this%20NFT%20on%20TaoDAONFT`;
  ...
  $('#soldCardEmailBtnInModal' + tokenAddress + id).click(()=>{
    window.location.href = `mailto:user@example.com?subject=Check%20out%20this%20NFT%20on%20TaoDAONFT&body=Never%20seen%20anything%20quite%20like%20this,%20${tokenPage}`;
  });
};
...
async function checkForApprovalBeforePuttingOnSale(tokenAddress, id){
  try{
    let approved = await TaoDAONFTTokenInstance.methods.isApprovedForAll(user.attributes.ethAddress, TaoDAONFTMarketplaceAddress).call();
...
function setApprovalIfNeeded(tokenAddress, id){
  $('#setApprovalBtn' + tokenAddress + id).click(async() =>{
    ...
    await TaoDAONFTTokenInstance.methods.setApprovalForAll(TaoDAONFTMarketplaceAddress, true).send({from: user.attributes.ethAddress}, (err, txHash) => {
      if(err){
        alert(err.message);
...
function putOnSale(tokenAddress, id, royalty, creator){
  $('#putOnSaleBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await TaoDAONFTMarketplaceInstance.methods.setOffer(amountInWei, id, tokenAddress).send({from: user.attributes.ethAddress});
...
function removeFromSale(tokenAddress, id, royalty, creator){
  ...
  $('#removeFromSaleBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await TaoDAONFTMarketplaceInstance.methods.removeOffer(id, tokenAddress).send({from: user.attributes.ethAddress});
...
function changePriceFrontEnd(tokenAddress, id, royalty, creator){
  $('#changePriceBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await TaoDAONFTMarketplaceInstance.methods.changePrice(amountInWei, id, tokenAddress).send({from: user.attributes.ethAddress});
...
function transferToken(tokenAddress, id){
  $('#transferTokenBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      await TaoDAONFTTokenInstance.methods.safeTransferFrom(user.attributes.ethAddress, toAddress, id).send({from: user.attributes.ethAddress});
