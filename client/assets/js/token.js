...
const TaoDAONFTTokenAddress = "0x6670E0C817B8c20F65a5820948A2cF67A9c60703"; //Kovan Network
...
async function buy(tokenAddress, id, price, royalty, creator){
  $('#buy' + tokenAddress + id).click(async function(){
    if(user){
      ...
      await TaoDAONFTMarketplaceInstance.methods.buyArt(id, tokenAddress).send({from: ethereum.selectedAddress, value: price}, (err, txHash) => {
        if(err){
          alert(err.message);
