...
async function mint(nftMetadataPath, nftPath){
  ...
  try {
    let receipt = await TaoDAONFTTokenInstance.methods.createArtwork(nftMetadataPath, royalty).send({from: user.attributes.ethAddress});
    $('#saveToIPFSBtn').prop('disabled', true);
...
async function uploadToDB(tokenId, nftMetadataPath, nftPath){
  $('#saveToIPFSBtn').html(`Finishing upload to TaoDAONFT <div class="spinner-border spinner-border-sm text-light" role="status">
                                                            <span class="sr-only">Loading...</span>
                                                          </div>`);
  try {
    ...
    artwork.set('tokenAddress', TaoDAONFTTokenAddress.toLowerCase());
    ...
    console.log(err);
    alert("Error uploading to TaoDAONFT");
    $('#saveToIPFSBtn').prop('disabled', false);
    $('#saveToIPFSBtn').html("Upload and Mint");
  }
};
...
function setArtForSale(tokenId){
  $('#setOffer').click(async() =>{
    ...
    try{
      await TaoDAONFTMarketplaceInstance.methods.setOffer(amountInWei, tokenId, TaoDAONFTTokenAddress).send({from: user.attributes.ethAddress});
